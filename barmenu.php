...
// Clear cart if the clear_cart parameter is set
if (isset($_GET['clear_cart']) && $_GET['clear_cart'] == 1) {
    $_SESSION['cart'] = array(); // Clear the cart
}

// Proceed with the rest of the page logic (like displaying the bar menu)
?>
...
<?php
    ...
    $status_query = "SELECT open_time, close_time, status FROM bar_time WHERE id = 1";
    ...
    *{
        ...
        <div class="fixed-header">
            <div class="container">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto">
                        <a href="clear_cart.php" class="btn btn-outline-primary">Food Menu
                        <i class="fas fa-arrow-right"></i>
                        </a>
                    </div>

                    <!-- <div class="col">
                    <a href="clear_cart.php">Food Menu</a>
                    </div>                -->
                </div>
            </div>
            <div class="container">
                    ...
                    <div class="col-6">
                        <?php
                            ...
                            if ($current_status == 'close') {
                                echo "<div class='alert-c' role='alert'>Bar Closed</div>";
                            } else {
                                echo "<div class='alert-o' role='alert'>Bar Open</div>";
                            }
                        ?>
                    </div>
                </div>
    ...
    <div class="row">
        <div class="col-md-12 ">
            ...
            if ($conn) {
                $cat_res = mysqli_query($conn, "SELECT * FROM barcategory WHERE status = 1 ORDER BY id ASC");
                if (!$cat_res) {
                    ...
                    <div class="menu-item scrollable-menu">
                        <li><a href='barmenu.php?cat_id=0' <?php if (!isset($_GET['cat_id']) || $_GET['cat_id'] == 0) echo "class='selected'"; ?>>All</a></li> <!-- Display 'All' category -->
                        <?php
                        while ($cat_row = mysqli_fetch_assoc($cat_res)) {
                            $cat_id = $cat_row['id'];
                            $selected_class = (isset($_GET['cat_id']) && $_GET['cat_id'] == $cat_id) ? "class='selected'" : "";
                            echo "<li><a href='barmenu.php?cat_id=$cat_id' $selected_class>" . $cat_row['category'] . "</a></li>";
                        }
                        ?>
                    </div>
    ...
    // Ensure $conn is not null before querying
    if ($conn) {
        $product_sql = "SELECT * FROM barmenu WHERE status = 1";
        ...
        if (isset($_GET['search_query']) && !empty($_GET['search_query'])) {
            ...
            $product_sql .= " AND (barmenu LIKE '%$search_query%' OR drink_detail LIKE '%$search_query%')";
        }
        $product_sql .= " ORDER BY drink DESC";
                ...
                ?>
                                <div class="card mb-3" >
                                    <div class="row g-0">
                                        <div class="col-8">
                                            <div class="card-body">
                                                <h5 class="card-title"><?php echo $product_row['drink'] ?></h5>
                                                <p class="card-text"><?php echo $product_row['drink_detail'] ?></p>
                                                <h5><b><?php echo $product_row['drink_price'] ?></b></h5>
                                            </div>
                                        </div>
                                        <div class="col-4 p-5 d-flex align-items-center">
        <?php if ($current_status == 'close'): ?>
            <div class="addToCartForm w-100" style="color: black;">
                <input type="hidden" name="drink_id" value="<?php echo $product_row['id']; ?>">
                <input type="hidden" name="drink_name" value="<?php echo $product_row['drink']; ?>">
                <input type="hidden" name="drink_price" value="<?php echo $product_row['drink_price']; ?>">
    ...
    <div id="cartMessagePopup">
        ...
        <button class="btn btn-primary" onclick="window.location.href='barcart.php'"><i class="fas fa-arrow-right fas fa-shopping-cart"></i> View</button>
    </div>
