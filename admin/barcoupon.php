...
if(isset($_GET['type']) && $_GET['type']!=='' && isset($_GET['id']) && $_GET['id']>0){
	...
	$id=$_GET['id'];
	if($type=='delete'){
		mysqli_query($conn,"delete from bar_coupon where id='$id'");
	...
	if($type=='active' || $type=='deactive'){
		...
		mysqli_query($conn,"update bar_coupon set status='$status' where id='$id'");
		redirect('barcoupon.php');
	}

}

$sql="select * from bar_coupon order by id desc";
...
$sql = "SELECT * FROM bar_coupon";
if ($search) {
    $sql .= " WHERE bar_coupon LIKE '%$search%'";
}
        ...
        <div class="overview-boxes">
            <div class="">
                <div class="right-side">
                    <a href="barcouponform.php"><button type="button" class="btn btn-primary">Add New Coupon</button></a>
                </div>
            </div>
        </div>
                    ...
                    <table>
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td>
                                <a href="barcouponform.php?id=<?php echo $row['id']?>"><label class="btn btn-success hand_cursor">Edit</label></a>&nbsp;
                                <?php
                                if($row['status']==1){  
                                ?>
