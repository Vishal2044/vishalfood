...
if(isset($_GET['id']) && $_GET['id']>0){
	$id=($_GET['id']);
	$row=mysqli_fetch_assoc(mysqli_query($conn,"select * from bar_coupon where id='$id'"));
...
if(isset($_POST['submit'])){
	...
	if($id==''){
		$sql="select * from bar_coupon where coupon_code='$coupon_code'";
	}else{
		$sql="select * from bar_coupon where coupon_code='$coupon_code' and id!='$id'";
	}	
	if(mysqli_num_rows(mysqli_query($conn,$sql))>0){
		...
		if($id==''){
			...
			mysqli_query($conn,"insert into bar_coupon(coupon_code,coupon_type,coupon_value,cart_min_value,expired_on,status,added_on) values('$coupon_code','$coupon_type','$coupon_value','$cart_min_value','$expired_on',1,'$added_on')");
		}else{
			mysqli_query($conn,"update bar_coupon set coupon_code='$coupon_code', coupon_type='$coupon_type' , coupon_value='$coupon_value', cart_min_value='$cart_min_value', expired_on='$expired_on' where id='$id'");
		}
		
		redirect('barcoupon.php');
	}
}
?>
    ...
    <div class="home-content">
      <div class="container-fluid">
          <div class="row">
              <div class="col">
                  <p>
                      <a href="barcoupon.php">Bar Coupon / </a> 
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form class="forms-sample" method="post">
                                ...
                                <button type="submit" class="btn btn-primary mt-3" name="submit" style="margin-left: 500px;">Submit</button>
                            </form>         
                        </div>
                    </div>
                </div>
