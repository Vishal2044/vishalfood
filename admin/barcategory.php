...
if (isset($_GET['type']) && $_GET['type'] !== '' && isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    $id = $_GET['id'];
    if ($type == 'delete') {
        mysqli_query($conn, "delete from barcategory where id='$id'");
        redirect('barcategory.php');
    }
    if ($type == 'active' || $type == 'deactive') {
        ...
        mysqli_query($conn, "update barcategory set status='$status' where id='$id'");
        redirect('barcategory.php');
    }
...
$sql = "SELECT * FROM barcategory  order by order_number";
$result = mysqli_query($conn, $sql);

$search = isset($_GET['search']) ? $_GET['search'] : '';

$sql = "SELECT * FROM barcategory";
if (!empty($search)) {
    $sql .= " WHERE barcategory LIKE '%$search%'";
}
  ...
  <section class="home-section">
    <nav>
      <div class="sidebar-button">
        ...
        <span class="dashboard">Bar Category</span>
      </div>

  <!-- Search Form -->
    <form method="GET" action="">
        <div class="search-box">
                <input type="text" name="search" class="form-control" placeholder="Search by barcategory	" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
    ...
    <div class="home-content">
      <div class="overview-boxes">
        <div class="">
          ...
          <a href="barcategoryform.php"><button type="button" class="btn btn-primary">Add barcategory</button></a>
          </div>
        </div>
      </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <td><?php echo $i ?></td>
                      ...
                      <td class="pt-3">
                        <a href="barcategoryform.php?id=<?php echo $row['id'] ?>"><label class="btn btn-success hand_cursor">Edit</label></a>&nbsp;
                        <?php
                            if ($row['status'] == 1) {
                          ?>
